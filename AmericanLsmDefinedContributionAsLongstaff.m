...
TrueCashFlow(:,end)=max(L0*(1+g)^T-L0*(Ss(:,end)./S0),0); 
for b =  (ActualTime-PeriodCell) : -PeriodCell : FirstCell

    Idx = find(L0*(1+g)^((b/PeriodCell)*Celldt).*exp(-rf*(ActualTime-b)/PeriodCell*Celldt) > L0*Ss(:,b+1)./S0);%Lonstaff : K > Ss(:,b+1)
    ...
    if b == (ActualTime-PeriodCell)
        ...
        for d = (b+PeriodCell) : PeriodCell : ActualTime
            [ExTime,~,~]=find(TrueCashFlow(Idx,d)~=0);                                    %
            Y1(ExTime,1)=TrueCashFlow(Idx(ExTime),d).*exp(-rf*((d-b)/PeriodCell)*Celldt);
        end
        Xx=Ss(Idx,b+1);   %
        X1=Xx;         %
        %------------------%
    ...
    Jdx = find( max(L0*(1+g)^((b/PeriodCell)*Celldt)-L0.*(Ss(Idx,b+1)./S0),0) > ContinValue) ;  %
    clear('ContinValue');
    TrueCashFlow(Idx(Jdx),b) = max(L0*(1+g)^((b/PeriodCell)*Celldt)-L0.*(Ss(Idx(Jdx),b+1)./S0),0);                   % 
...
for DateCount = FirstCell : PeriodCell : ActualTime
    ...
    TempPrice1=sum(PeriodDebtValue.*exp(-rf*((DateCount/PeriodCell)*Celldt))); %
