...
Celldt = 13/52;         %
%--------------%
FirstRetireTime=FirstRetire*PeriodCell;
%-----------%
corrAB=0;
s=1;
v=2;   
...
%---------%
IndexA=zeros(SimulateTimes,TimeCell);
IndexA(:,1)=1; % T=0 Invest A assets
IndexB=zeros(SimulateTimes,TimeCell);
%--------%
...
for d = FirstCell : PeriodCell : ActualTime-PeriodCell
        ...
        IndexA(SwitchToA,(d+PeriodCell)/PeriodCell) = 1;
        IndexB(SwitchToA,(d+PeriodCell)/PeriodCell) = 1;
        ...
        [ForntAssetIsBb,~,~]=find(ReturnB(SwitchToA,d/PeriodCell)==log(RBb(SwitchToA,d+1)./RBb(SwitchToA,d-PeriodCell+1)));
        TotalswitchAssetB=size(ForntAssetIsBb,1)+TotalswitchAssetB;       
        TempswitchAssetB=size(ForntAssetIsBb,1)+TempswitchAssetB;
        ...
        [ForntAssetIsAa,~,~]=find(ReturnB(SwitchToB,d/PeriodCell)==log(RAa(SwitchToB,d+1)./RAa(SwitchToB,d-PeriodCell+1)));       
        TotalswitchAssetB=size(ForntAssetIsAa,1)+TotalswitchAssetB;
        TempswitchAssetB=size(ForntAssetIsAa,1)+TempswitchAssetB;
        [NoSwitch,~,~]=find(EA(:,d/PeriodCell)==EB(:,d/PeriodCell));
        [ForntAssetIsA,~,~]=find(ReturnA(NoSwitch,d/PeriodCell)==log(RAa(NoSwitch,d+1)./RAa(NoSwitch,d-PeriodCell+1)));
        [ForntAssetIsAa,~,~]=find(ReturnB(NoSwitch,d/PeriodCell)==log(RAa(NoSwitch,d+1)./RAa(NoSwitch,d-PeriodCell+1))); 
        ReturnA(ForntAssetIsA,(d+PeriodCell)/PeriodCell)    =   log(RAa(NoSwitch,d+PeriodCell+1)./RAa(NoSwitch,d+1));
        ReturnB(ForntAssetIsAa,(d+PeriodCell)/PeriodCell)    =   log(RAa(NoSwitch,d+PeriodCell+1)./RAa(NoSwitch,d+1));
        IndexA(ForntAssetIsA,(d+PeriodCell)/PeriodCell) = 1;
        IndexB(ForntAssetIsAa,(d+PeriodCell)/PeriodCell) = 1;
        %-----%
        [ForntAssetIsB,~,~]=find(ReturnA(NoSwitch,d/PeriodCell)==log(RBb(NoSwitch,d+1)./RBb(NoSwitch,d-PeriodCell+1)));  
        [ForntAssetIsBb,~,~]=find(ReturnB(NoSwitch,d/PeriodCell)==log(RBb(NoSwitch,d+1)./RBb(NoSwitch,d-PeriodCell+1))); 
        ReturnA(ForntAssetIsB,(d+PeriodCell)/PeriodCell)     =   log(RBb(ForntAssetIsB,d+PeriodCell+1)./RBb(ForntAssetIsB,d+1)); 
        ReturnB(ForntAssetIsBb,(d+PeriodCell)/PeriodCell)    =   log(RBb(ForntAssetIsBb,d+PeriodCell+1)./RBb(ForntAssetIsBb,d+1)); 
...
%--------------------------%
for b =  (ActualTime-PeriodCell) : -PeriodCell : FirstRetireTime
            ...
            if b == (ActualTime-PeriodCell)
                    ...
                    Y1Aa=gpuArray(Y1A);
                   [InvestA,~,~]=find(IndexA(IdxA,b/PeriodCell)==1);      
                   [InvestB,~,~]=find(IndexA(IdxA,b/PeriodCell)==0);  
                    X1A(InvestA)=PeriodReturnA(InvestA,b/PeriodCell).*S0A;          
                    X1A(InvestB)=PeriodReturnA(InvestB,b/PeriodCell).*S0B; 
                    %------------%
                   ...
                   [InvestAa,~,~]=find(IndexB(IdxB,b/PeriodCell)==1);      
                   [InvestBb,~,~]=find(IndexB(IdxB,b/PeriodCell)==0);  
                    X1B(InvestAa)=PeriodReturnB(InvestAa,b/PeriodCell).*S0A;          
                    X1B(InvestBb)=PeriodReturnB(InvestBb,b/PeriodCell).*S0B;               
                    %------------%
...
nnB=0;
for DateCount = FirstRetireTime : PeriodCell : ActualTime
