...
for t=1:1:1
    ...
    for b = FirstCell : PeriodCell : ActualTime-PeriodCell
        Y1a(:,b/PeriodCell)=SAa(:,b+PeriodCell+1); %由於有考慮第0周 , 故這裡使用b+1 ,起點是從第1期開始
        X1a(:,b/PeriodCell)=SAa(:,b+1); %由於考慮第0周 價格,故這裡起點是從第0周開始  EX: b=1 , Y1=第1周股價,X1=第0周股價
        R1a =[ones(size(X1a(:,b/PeriodCell)))  (exp(-X1a(:,b/PeriodCell)./2)).*ones(size(X1a(:,b/PeriodCell)))  (exp(-X1a(:,b/PeriodCell)./2)).*(1.-X1a(:,b/PeriodCell))  (exp(-X1a(:,b/PeriodCell)./2)).*(1.-(2.*X1a(:,b/PeriodCell))+(X1a(:,b/PeriodCell).^2))]; %自變數X為各期股價  , 用Laguerre多項式,項數至第二項作為最適LSM回歸多項式
        a1 = (R1a')*R1a;
        a2 = a1\(R1a');
    ...
    for d = FirstCell : PeriodCell : ActualTime-PeriodCell
         EA(:,d/PeriodCell)=log(YhatA(:,d)./SAa(:,d+1));
         EB(:,d/PeriodCell)=log(YhatB(:,d)./SBb(:,d+1));% 第t周預期報酬=第t-1周預測第t周的價格/第t-1周價格 (這邊SA要加一是因為有考慮t=0期的價格 故t=1時 t+1=第一期的價格 以此類推)
    end
       %步驟五    
     for c = 1:1:SimulateTimes
        for d = FirstCell:PeriodCell:ActualTime-PeriodCell
            if EA(c,d/PeriodCell)>EB(c,d/PeriodCell)
                ReturnA(c,d/PeriodCell)=log(RAa(c,d+PeriodCell+1)/RAa(c,d+1));
                ReturnB(c,d/PeriodCell)=log(RAa(c,d+PeriodCell+1)/RAa(c,d+1));
            elseif EB(c,d/PeriodCell)>EA(c,d/PeriodCell)
                ReturnA(c,d/PeriodCell)=log(RBb(c,d+PeriodCell+1)/RBb(c,d+1));
                ReturnB(c,d/PeriodCell)=log(RBb(c,d+PeriodCell+1)/RBb(c,d+1));
            elseif EA(c,d/PeriodCell)==EB(c,d/PeriodCell)
                if d>FirstCell
                    if     EA(c,d-PeriodCell)==EA(c,d)
                            ReturnA(c,d/PeriodCell)=log(RAa(c,d+PeriodCell+1)/RAa(c,d+1));
                            ReturnB(c,d/PeriodCell)=log(RAa(c,d+PeriodCell+1)/RAa(c,d+1));
                    elseif EB(c,d-PeriodCell)==EB(c,d)
                            ReturnA(c,d/PeriodCell)=log(RBb(c,d+PeriodCell+1)/RBb(c,d+1));
                            ReturnB(c,d/PeriodCell)=log(RBb(c,d+PeriodCell+1)/RBb(c,d+1));
                    end
                else
